...
if (ec == null){   
	...
	pageContext.forward(request.getContextPath()+ "/seleccion/impresionMasiva.do");  
	return;
}
...
session.setAttribute("form.back","/consulta/impresionMasiva");
...
function __return(){
	document.location="<%=request.getContextPath()%>/seleccion/impresionMasiva.do";
}
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="2" class="linea-botton">
			  <tr> 
				<td width="92%"><div align="left" class="login">
					<bean:message key="lbl.compensacion.impresion.titulo"/> - 
					<%
	...
	<tr>
		<td height="15" colspan="2" nowrap>
			<div align="left" class="texto-acceso">
				<br>
				<bean:message key="lbl.cc.descripcion.observacion"/>
				<br>
			</div>
		</td>
	</tr>
...
<br>
<html:form action="/compensacion/impresion/multiple" method="post">
<table width="100%" border="0" cellpadding="1" cellspacing="2">
	<tr>
  		<td>
      		<div align="left">
      			<br>
      			<input type="button" class="botton" value="<bean:message key="lbl.compensacion.impresion"/>" onclick="__submit();">
			</div>
		</td>
	</tr>
	<tr><td>&nbsp;</td></tr>
	<tr>
		<td>
			<input type="checkbox" name="_all" onclick="__checkAll(this);">
			&nbsp;
			<bean:message key="lbl.compensacion.autorizar.seleccionar.todos"/>
		</td>
	</tr>
</table>
<table width="100%" border="0" cellpadding="1" cellspacing="2">
...
%>
	<TR valign="top" <%=styleClass%>>
		<TD class="bienvenida" align=left width="14%">
	    	...
	    	&nbsp;&nbsp;&nbsp;
	    	<%} %>
	    	<%=f.formatFecha(m.getFechaValida(), locale)%>
	    </TD>
	    ...
	    <TD class="bienvenida" style="color:blue;CURSOR: hand" 
			...
			<%}%>
	    	align=left width="30%"><SPAN id=desc style="color:blue;hand;font-size:10px;font-family: Verdana, Arial, Helvetica, sans-serif;"><%=m.getDescripcion()%></SPAN>
	    </TD>
	    <%}
		else{  // si tipo == D se crea Nuevo link %>
		<TD class="bienvenida" 
	    	align=left width="30%" style="color:blue;CURSOR: hand" onclick="location.href='<%=request.getContextPath() %>/detalles/movimiento.do?secuencia=<%=m.getSecuencia() %>'"><SPAN id=desc style="color:blue;hand;font-size:10px;font-family: Verdana, Arial, Helvetica, sans-serif;"><%=m.getDescripcion()%></SPAN>
	    </TD>
		<%} %>
	    ...
	    <TD class="bienvenida" align=right><%=m.getEstado()==null?"":m.getEstado() %></TD>
	</TR>
	<tr>
		<td colspan="5">
...
%>
			<div id="iddeta<%=m.getSecuencia()%>" sytle="display:none;">
				<table align="right" width="95%" border="0" cellpadding="1" cellspacing="2">
					<tr class="tabla-acceso">
						<td class="texto-acceso" width="5%">Seleccionar</td>		               
					   	<td class="texto-acceso" width="15%">Valor Total del Formulario</td>
					   	<td class="texto-acceso" width="15%">Número Declaración</td>
...
%>
					<tr class="tabla-acceso">
						<td class="bienvenida" align="center"><html:checkbox property="indice" value="<%=d.getRuta()%>"/></td>
...
function __checkAll(element) {
	var elements = document.forms['impresionMultipleForm'].elements;
...
function __submit() {
	...
	var pdfPrint = "";
	var allPdfPrint = "";
	var pathDominio = location.protocol + "//" + location.host;
	var elements = document.forms['impresionMultipleForm'].elements;
	//var pathPDF = "[" + pathDominio + "/PDFHistory/{0}]";
	var pathPDF = "[http://192.168.242.220:81/PDFHistory/{0}]";
	var pathImpresion = pathDominio + "/internet/PDFPrinter/silent_print_pdf.jsp?DOC_LIST=";
	
	for(var i = 0; i < elements.length; i++) {
		if (elements[i].name == "indice" && elements[i].checked) {
			flag = true;
			index = elements[i].value;
			count++;
			pdfPrint = pathPDF.format(index);
			allPdfPrint = allPdfPrint + pdfPrint;
		}
	}
	URLFinal = pathImpresion + allPdfPrint;
	if (count > 0){
		showWindow(URLFinal);
		document.forms['impresionMultipleForm'].submit();
	} else {
		alert("Debe seleccionar al menos un registro para Imprimir");
	}
}

function changeIframeSrc(urlPrint) {
	document.getElementById('cible').src=urlPrint;
} 

function showWindow(url) {
	windowPrint = window.open(url, "_blank", "location=1,status=1,scrollbars=1,width=400,height=350");
...
<tr> 
	<td valign="top" class="texto-acceso"><div align="center"><br>
			<INPUT class="botton" onclick="document.location='<html:rewrite page='/seleccion/impresionMasiva.do'/>'" type=button value="<bean:message key="lbl.retroceder"/>"></div></td></tr>
</table>
<%
}
%>
