...
%>
<%
Object objEstCta = session.getAttribute("movimiento-detalle");
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="2" class="linea-botton">
			  <tr> 
				...
				<td width="8%"><div align="right">&nbsp;
							<INPUT class="botton" onclick="document.location='<html:rewrite page='/compensacion/consulta2.jsp'/>'" type="button" value="<bean:message key="lbl.retroceder"/>"></div></td>
			  </tr>
			</table></div></td>
	  </tr>
	  <tr> 
			  ...
			  <table align="right" width="100%" border="0" cellpadding="1" cellspacing="2">
				   <tr class="tabla-acceso">		               
					...
					<%for (int i = 0; i < mov.size(); i++){
						...
						%>		
				<tr class="tabla-acceso">				   
				   <td class="bienvenida" align="right"><%=f.formatMonto(m.getValor())%></td>				 				   
				   <td class="bienvenida" align="center"><%=m.getSecuenciaDeclaracion()%></td>		
				   <td class="bienvenida"><%=getDescEstado(m.getEstado())%></td>
				   <td class="bienvenida"><% if (m.getNombreFormulario() != null){%>
						<a  href="javascript:_submit(<%=i%>);"><%=m.getCodigoFormulario()%>-<%=m.getNombreFormulario()%></a>
					<%}else{%>
						&nbsp;
					<%}%></td>										
				</tr>
...
<html:form action="/consultar/formulario/reprocesado" method="post" target="_formulario">
	<html:hidden property="indice"/>
	<!--<html:hidden property="indice2"/>-->
	<html:hidden property="cuenta" value="<%=session.getAttribute("cuenta.csic").toString()%>"/>
	<html:hidden property="est" value="."/>
...
<table width="80%" border="0" cellpadding="0" cellspacing="2">
	...
	<tr> 
		<td valign="top" class="texto-acceso" colspan="2"><div align="center"><br>
				<INPUT class="botton" onclick="document.location='<html:rewrite page='/compensacion/consulta2.jsp'/>'" type=button value="<bean:message key="lbl.retroceder"/>"></div></td></tr>
</table>					
<%
}
else
