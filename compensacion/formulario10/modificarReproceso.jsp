...
if (ec == null){   
	...
	pageContext.forward(request.getContextPath()+ "/formulario10/mant/modificar.do");  
	return;
}
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="2" class="linea-botton">
			  <tr> 
				<td width="92%"><div align="left" class="login">
					Reproceso de Relación de Operaciones de Cuentas de Compensación
					<%java.text.SimpleDateFormat df = new java.text.SimpleDateFormat("MMMM-yyyy",locale);
					java.util.Date inicio = df.parse((String)session.getAttribute("periodo.consulta"));
					...
					session.setAttribute("fecha.desde.formulario.10", df.format(inicio));
                            session.setAttribute("fecha.hasta.formulario.10", df.format(fin));%>
					</div></td>
				<td width="8%"><div align="right">&nbsp;
							<INPUT class="botton" onclick="document.location='<html:rewrite page='/formulario10/mant/reproceso.do'/>'" type="button" value="<bean:message key="lbl.retroceder"/>"></div></td>
			  </tr>
			</table></div></td>
	  </tr>
	  <tr> 
...
<br>
<table width="70%" border="0" cellpadding="1" cellspacing="2" align="left">

<tr class="tabla-acceso">
 <td class="texto-acceso">Periodo</td>
	<td class="texto-acceso"><bean:message key="lbl.compensacion.consulta.estado.nombre"/></td>
	<td class="texto-acceso" align="center">Estado</td>
 </tr> 
<%
String styleClass = "";
String est = null;
...
<TR valign="top" <%=styleClass%>>
    <td class="bienvenida"><%= sdf.format(m.getFechamovimiento()) %></td>
    <TD class="bienvenida">
	<% if (m.getFormulario() != null){%>
		<a  href="javascript:_submit(<%=i%>);"><%=m.getFormulario() %> - <%=m.getNombre()%></a>
	<%}else{%>
		&nbsp;
	<%}%></TD>
	<td class="bienvenida" align="center"><%=estado %></td>
	</TR>
<%} /* End for - loop*/ %>
...
<html:form action="/formulario10/reprocesar/formulario" method="post" target="_formulario">
	<html:hidden property="indice"/>
...
<table width="80%" border="0" cellpadding="0" cellspacing="2">
	...
	<tr> 
		<td valign="top" class="texto-acceso" colspan="2"><div align="center"><br>
				<INPUT class="botton" onclick="document.location='<html:rewrite page='/formulario10/mant/reproceso.do'/>'" type=button value="<bean:message key="lbl.retroceder"/>"></div></td></tr>
</table>					
<%
}
else
