...
private String getDescEstado(String estado){
    if ("P".equals(estado)){
        return "Guardado Temporalmente";
    }
    else if ("M".equals(estado)){
        return "Pendiente por Elaborar";
    }
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="2" class="linea-botton">
			  <tr> 
				...
				<td width="8%"><div align="right">&nbsp;
							<INPUT class="botton" onclick="document.location='<html:rewrite page='/mant/reprocesar1.do'/>'" type="button" value="<bean:message key="lbl.retroceder"/>"></div></td>
			  </tr>
			</table></div></td>
	  </tr>
	  <tr> 
...
<%}%>
<html:form action="/reproceso" method="post"> <!-- /asociar-->
...
<tr class="tabla-acceso">
    ...
    <td class="texto-acceso" width="30%"><bean:message key="lbl.descripcion"/></TD>
...
<TR valign="top" <%=styleClass%>>
    ...
    align=left width="30%"><SPAN id=desc style="color:blue;hand;font-size:10px;font-family: Verdana, Arial, Helvetica, sans-serif;"><%=m.getDescripcion()%></SPAN></TD>
        <TD class="bienvenida" align=right width="5%"><%="D".equals(m.getTipoTransaccion())?f.formatMonto(m.getMonto()):"&nbsp;"%></TD>
    <TD class="bienvenida" align=right width="5%"><%="C".equals(m.getTipoTransaccion())?f.formatMonto(m.getMonto()):"&nbsp;"%></TD>
...
<tr>
	  ...
	  %>
           <div id="iddeta<%=m.getSecuencia()%>" sytle="display:none;">
			  <table align="right" width="100%" border="0" cellpadding="1" cellspacing="2">
				   <tr class="tabla-acceso">		               
					   ...
					   <td class="texto-acceso" width="25%">Formulario</td>						   
					   <td class="texto-acceso" width="25%">Formulario Actual</td>
				...
				<tr class="tabla-acceso">				   
				   ...
				   <td class="bienvenida"><%=getDescEstado(d.getEstado())%></td>
				   <td class="bienvenida"><% if (d.getNombreFormulario() != null){%>
					...
					<%}else{%>
						&nbsp;
					<%}%></td>
					<td class="bienvenida"><% if (d.getNombreFormularioActual() != null){%>
						<a  href="javascript:_formulario(<%=i%>,<%=j%>);"><%=d.getCodigoFormularioActual()%>-<%=d.getNombreFormularioActual()%></a>
					<%}else{%>
						&nbsp;
					<%}%></td>
				</tr>
...
function _formulario(index, index2){
...
<html:form action="/consultar/formularioActual" method="post" target="_formulario">
	<html:hidden property="indice"/>
	<html:hidden property="indice2"/>
	<html:hidden property="cuenta" value="<%=session.getAttribute("cuenta.csic").toString()%>"/>
	<html:hidden property="est" value="."/>
	<html:hidden property="unico" value="RA"/>
</html:form>
<table width="80%" border="0" cellpadding="0" cellspacing="2">
	...
	<tr> 
		<td valign="top" class="texto-acceso" colspan="2"><div align="center"><br>
				<INPUT class="botton" onclick="document.location='<html:rewrite page='/mant/reprocesar.do'/>'" type=button value="<bean:message key="lbl.retroceder"/>"></div></td></tr>
</table>					
<%
}
else
...
<tr> 
	<td valign="top" class="texto-acceso"><div align="center"><br>
			<INPUT class="botton" onclick="document.location='<html:rewrite page='/mant/modificar.do'/>'" type=button value="<bean:message key="lbl.retroceder"/>"></div></td></tr>
</table>
<%
}
%>
