...
function closeallmenus(){ showListBoxes(); om=0;if(ns4){omv.visibility="hide"}for(a=1;a<menus;a++){menu=eval("menu"+a);mi=gmstyle("menu"+a);if (menu[6]!=1){mi.visibility='hidden'}else{am=""}}hll="";close_el();}
function rep_img(){if(timo){timo.src=o_img}}
function hideMenu(){
	for(a=1;a<menus;a++){
		menu=eval("menu"+a);mi=gmstyle("menu"+a);
...
function dmenu(mnu){
	...
	if(ns4)
	{
		...
		if (hlp[11]){mt+="<layer "+" top=0 left=0 height=2 width=100%></layer>"}mt+="<layer id="+mnu+" top=0 left=0 height=1 width=2></layer>"}
	else
	{
...
if(!ns61&ns6){window.onload=Minit}else{Minit();}