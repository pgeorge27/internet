...
if (CONST_X === undefined) {
  var CONST_X=0;
}
if (CONST_Y === undefined) {
  var CONST_Y=0;
}
...
var iShiftClockDelay = 30;      // delay before transmitting clock
var iDataDelay  = 40; //time clock stable during data bit(s) transmission - this is the half period
...
var htmlElementId = "captorsTransmissionZone";
...
)
{
  ...
  */
    ...
    Background = rectangle((x-_bordersize),
                         (y-_bordersize),
                         (captorWidth*numberOfCaptors),
                         (captorHeight),
                         _backgroundcolor,
                         _backgroundcolor);


    ledContainer = document.getElementById(htmlElementId);
    var style = ledContainer.style;
    style.width = (captorWidth)*numberOfCaptors + "px";
    style.height = captorHeight+ "px";
    

    for ( i=0;i<numberOfCaptors;i++)
    {

        captorsTransmissionZone[i] = rectangle((x+(captorWidth)*i),
                                               y,captorWidth,captorHeight,
                                               oneColor,oneColor,'');

        ledContainer.appendChild(captorsTransmissionZone[i]) ;
...
//		captorWidth = Math.round((iWidth)/captorsNumber);
		...
		initializeTransmissionArea(captorsNumber,
									0,CONST_Y,borderSize,captorWidth,
