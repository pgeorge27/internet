...
<table width="750" border="0"  cellpadding="0" cellspacing="0">
				  ...
				  <td width="68%" height="20" class="catra-texto" align="left">
					<table width="80%" border="0" cellpadding="0" cellspacing="2">
					  <tr class="tabla-acceso"> 
						<td width="40%" height="15" class="texto-acceso"><bean:message key="lbl.administration.user.name"/></td>
						<td width="30%" height="15" class="texto-acceso"><bean:message key="lbl.administration.status"/></td>
						<td width="10%" class="texto-acceso"><bean:message key="lbl.administration.user.username"/></td>
						<td width="20%"></td>
					</tr>
	<%
	...
	%>
         ...
         <tr <%=i%2==0?"class=\"celda-clave1\"" : ""%>>
			<td class="bienvenida"><%=u.getNombreCompleto()%></td>
			<td class="bienvenida"><%=TagUtil.getString(pageContext, getStatus(u.getUsuarioGrupo().getEstado()))%></td>
			<td class="bienvenida"><%=u.getUsuarioGrupo().getUserId()%></td>
			<td class="bienvenida" align="center" nowrap><span><input type="button" class="botton-acceso" name="btnChange" value="<bean:message key="lbl.administration.disable"/>" onclick="__submit('<%=u.getUsuarioGrupo().getUserId()%>', 'I')"></span>
								  			   <span><input type="button" class="botton-acceso" name="btnChange" value="<bean:message key="lbl.administration.unlock"/>" onclick="__submit('<%=u.getUsuarioGrupo().getUserId()%>', 'D')"></span></td>
		</tr>	
      <% }
	    }%>
